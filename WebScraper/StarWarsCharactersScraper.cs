...
namespace NameGeneratorAPI.WebScraper
{
    public class StarWarsCharactersScraper
    {
        ...
        public List<StarWarsCharacter>? Characters { get; set; }
        ...
        public List<StarWarsCharacter> RetrieveCharacters() {
            ...
            if(speciesGroupingsNodes == null || !speciesGroupingsNodes.Any()) return new List<StarWarsCharacter>();

            IEnumerable<StarWarsCharacter> characters = speciesGroupingsNodes.SelectMany(x => CreateCharactersFromSpeciesGrouping(x));
            Characters = characters.ToList();
            return Characters;
        }

        private static IEnumerable<StarWarsCharacter> CreateCharactersFromSpeciesGrouping(HtmlNode speciesGroupingNode) {
            ...
            if(categoryNodes == null || !categoryNodes.Any()) return new List<StarWarsCharacter>();

            IEnumerable<StarWarsCharacter> characters = categoryNodes.SelectMany(x => CreateCharactersFromCategory(speciesGroupingName, x));
            return characters;
        }

        private static IEnumerable<StarWarsCharacter> CreateCharactersFromCategory(string speciesGroupingName, HtmlNode categoryNode) {
            ...
            if(speciesNodes == null || !speciesNodes.Any()) return new List<StarWarsCharacter>();
        ...
        private static IEnumerable<StarWarsCharacter> CreateCharactersFromSpecies(string speciesGroupingName, string speciesName, string characterSelector, HtmlNode categoryNode) 
        {
            ...
            if(characterNodes == null || !characterNodes.Any()) return new List<StarWarsCharacter>();

            IEnumerable<StarWarsCharacter> characters = characterNodes.Select(x => CreateCharacter(speciesName, speciesGroupingName, categoryName, x));
            return characters;

        }
        private static StarWarsCharacter CreateCharacter(string species, string speciesGrouping, string category, HtmlNode characterNode) {
